...
class Bill extends Model
{
    ...
    public function getBillListGroup($admin_id)
    {
        $retObj = DB::table('transaction as a')
            ...
            ->select(DB::raw('paid_date,a.employee_id,group_concat(a.narrative) as narrative,sum(amount) as amount,max(v.name) as name,max(transaction_id) as transaction_id,max(v.account_no) as account_no,max(v.account_holder_name) as account_holder_name'))
            ->get();
        return $retObj;
    }
